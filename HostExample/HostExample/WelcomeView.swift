...
struct WelcomeView: View {
    ...
    @State private var selectedSampleRate: Int = 16000
    ...
    private func load() {
        ...
        Task {
            do {
                ...
                mockData = try await MockDataService.fetch(sampleRate: selectedSampleRate)
                isLoading = false
        ...
        NavigationStack {
            VStack(spacing: 30) {
                VStack(spacing: 0) {
                    ...
                    Text("Host Driving Service Mode")
                        .font(.caption)
                ...
                Button(action: {
                    ...
                    Text(isLoading ? "Loading..." : "Load")
                        ...
                        .padding(.vertical, 12)
                        .background(avatarID.isEmpty ? Color.gray : Color.blue)
                        .cornerRadius(10)
                }
                .disabled(avatarID.isEmpty || isLoading)
                .padding(.horizontal, 16)
                .padding(.vertical, 50)
            ...
            .navigationDestination(isPresented: $navigateToContent) {
                if let avatar, let mockData {
                    ContentView(avatar: avatar, mockData: mockData, sampleRate: selectedSampleRate)
                }
            }
        }
    }
